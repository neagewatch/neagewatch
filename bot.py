...
# 日付抽出ユーティリティ
# =====================
def parse_rss_date(item) -> str:
    """RSS の pubDate / dc:date を YYYY-MM-DD で返す"""
    for tag_name in ["pubDate", "date", "dc:date", "published", "updated"]:
        tag = item.find(tag_name)
        if tag and tag.text:
            raw = tag.text.strip()
            # RFC822: Mon, 23 Jun 2026 10:00:00 +0900
...
def extract_change_date(text: str, fallback: str) -> str:
    """記事本文から実施日（〜から/〜より 値上げ）を抽出。
    『6月1日から』『2026年4月より』などのパターンを探す。"""
    current_year = NOW.year

    # パターン1: 2026年4月1日から / より
    m = re.search(r"(\d{4})年\s*(\d{1,2})月\s*(\d{1,2})日(?:から|より|以降|実施|改定|出荷分)", text)
    if m:
        return f"{int(m.group(1)):04d}-{int(m.group(2)):02d}-{int(m.group(3)):02d}"

    # パターン2: 4月1日から（年なし → 当年または翌年を推定）
    m = re.search(r"(\d{1,2})月\s*(\d{1,2})日(?:から|より|以降|実施|改定|出荷分)", text)
    if m:
        month, day = int(m.group(1)), int(m.group(2))
        year = current_year
        # 過去すぎる月なら翌年の値上げと推定（1〜3月で現在が後半など）
...
def main():
    ...
    for item in all_items:
        ...
        change_date = extract_change_date(article_text, article_date)
        ...
        try:
            ...
            inserted += 1
            print(f"✅ INSERT: {company['name']} {product[:30]} {old_price}→{new_price} ({change_date})")
        except Exception as e:
